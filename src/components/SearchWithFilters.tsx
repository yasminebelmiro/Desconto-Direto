...
import Filters from "../pages/Consumer/components/Filters.tsx";
import ListOffers from "../pages/Consumer/components/ListOffers.tsx";
import NotFoundItem from "./NotFoundItem.tsx";
import type { OfferTypes } from "../types/OfferTypes.ts";
interface SearchWithFiltersProps {
  offers: OfferTypes[];
  loading: boolean;
}
const SearchWithFilters = ({ offers, loading }: SearchWithFiltersProps) => {
  const [search, setSearch] = useState("");
  const [currentPage, setCurrentPage] = useState<number>(1);
  const [order, setOrder] = useState<string>("exp-asc");
